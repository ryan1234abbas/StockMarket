...
class DetectionWorker(QThread):
    ...
    def analyze_candles_tm(self, left_img, boxes_3020, labels_3020, scores_3020,
        ...
        # === BACKWARD MOVEMENT DETECTION (from v3) ===
        ...
        current_rml_1510 = rightmost_lbl_1510  # Define here for use in condition
        ...
        # Current labels for SRL logic (current_rml_1510 already defined on line 314)
        ...
        # === PRIMARY TRADE: RML with candle alignment (from v3) ===
        ...
        # === SRL BACKUP TRADE with pattern duplicate check (from v3) ===
        if (not self.rml_backward_lockout and
            not self.pending_trade and
        ...
        with mss.mss() as sct:
            try:
                while self.running:
                    ...
                    # Process yellow results SEPARATELY - KEY FIX
                    ...
                    # DO NOT append yellow_label to merged_right_labels - KEY FIX
                ...
                if l == "candle" and candle_scores[i] >= scandle_conf]
                    ...
                    decision = self.analyze_candles_tm(
                        ...
                        yellow_detected=yellow_detected  # Pass as explicit parameter
                    )
