...
class DetectionWorker(QThread):
    ...
    def analyze_candles_tm(self, left_img, boxes_3020, labels_3020, scores_3020,
            ...
            if (current_rml_1510 != self.prev_rml_1510 and not self.rml_backward_lockout):
                self.srl_lockout_after_trade = False
                # Note: We no longer clear last_executed_pattern here
                # The label ID system will handle duplicate detection
            ...
            # BUY condition with label ID duplicate check
            if (not self.rml_backward_lockout and 
                ...
                if candle_aligned:
                    # Generate pattern signature using label ID
                    pattern_sig = self.get_pattern_signature_by_id(
                        rightmost_lbl_3020, rightmost_lbl_1510, box_1510, is_srl=False
                    )
                    
                    # Check if we already traded this EXACT pattern AND it hasn't evolved since
                    ...
                    # Pattern is either new OR has evolved - Execute trade
            ...
            # SELL condition with label ID duplicate check
            ...
            # === SRL BACKUP TRADE with label ID duplicate check ===
            if (not self.rml_backward_lockout and
                not self.pending_trade and
                ...
                if (current_srl_1510 == "HL" and 
                    ...
                    # Generate SRL pattern signature using SECOND label's box for ID
                    pattern_sig = self.get_pattern_signature_by_id(
                        rightmost_lbl_3020, current_srl_1510, box_second_1510, is_srl=True
                    )
                    
                    # Check if we already traded this EXACT SRL pattern AND it hasn't evolved since
                    if pattern_sig == self.last_executed_pattern and not self.pattern_has_evolved:
                    ...
                    decision = "BUY"
                    self.srl_lockout_after_trade = True
                    self.last_executed_pattern = pattern_sig  # Save this SRL pattern with label ID
                    self.pattern_has_evolved = False  # Reset evolution flag after trading
                    self.last_traded_1510_label = current_srl_1510  # Track the label string
                    self.frames_since_last_trade = 0  # Reset frame counter
                    
                    pyautogui.hotkey('ctrl','b')
                    print(f"BUY executed - SRL Pattern with Label ID: {pattern_sig}")
                    return decision
                ...
                # SRL SELL condition
                    ...
                    pyautogui.hotkey('ctrl','m')
                    print(f"SELL executed - SRL Pattern with Label ID: {pattern_sig}")
                    return decision
