...
class PIXEL_TRADER:
    ...
    def __init__(self, colors_to_check):
        self.utils = Utils()
        self.EDGE_DELTA = 5
        self.WIDTH = 100
        ...
        for color in colors_to_check:
            if color == 'mb':
                ...
                self.green_lines_list.append(self.utils.gold_bar)
        ...
        for color in colors_to_check:
            if color == 'mb':
                ...
                self.purple_lines_list.append(self.utils.gold_bar)
    ...
    def old_buy_or_sell(self):
        ...
        while self.keystroke == key:
            ...
            if all([self.utils.check_color_in_all_pixels(screenshot_array, color) for color in self.green_lines_list]):
                while self.keystroke == key:
                    screenshot_array = np.array(pyautogui.screenshot(region=(self.top_pixel[1] - self.WIDTH, self.top_pixel[0], self.WIDTH, self.bottom_pixel[0] - self.top_pixel[0])))
                    if all([self.utils.check_color_in_all_pixels(screenshot_array, color) for color in self.purple_lines_list]):
                        self.utils.sell()
        ...
        while self.keystroke == key and self.utils.STATUS is None:
            ...
            if all(purple_list):
                ...
                while self.keystroke == key:
                    screenshot_array = np.array(pyautogui.screenshot(region=(self.top_pixel[1] - self.WIDTH, self.top_pixel[0], self.WIDTH, self.bottom_pixel[0] - self.top_pixel[0])))
                    green_list = [self.utils.check_color_in_all_pixels(screenshot_array, color) for color in self.green_lines_list]
                    if all(green_list) and not self.utils.check_color_in_all_pixels(screenshot_array, self.utils.purple_bg):
                ...
                while self.keystroke == key:
                    screenshot_array = np.array(pyautogui.screenshot(region=(self.top_pixel[1] - self.WIDTH, self.top_pixel[0], self.WIDTH, self.bottom_pixel[0] - self.top_pixel[0])))
                    purple_list = [self.utils.check_color_in_all_pixels(screenshot_array, color) for color in self.purple_lines_list]
                    if all(purple_list) and not self.utils.check_color_in_all_pixels(screenshot_array, self.utils.green_bg):
...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        print('Usage: python mokhtiyar.py <colors_to_check>')
        print('colors_to_check options: Any combo of: "mb", "st", "lt", "bg"')
        print('Example: py mokhtiyar.py "buy&sell" "mb", "st", "lt"')
        sys.exit(1)

    colors_to_check = sys.argv[1:] # All arguments from argv[2] onwards

    if all(line not in ['mb', 'st', 'lt', 'bg', 'gp'] for line in colors_to_check):
        print('Usage: python chupke_mokhtiyar.py <colors_to_check>')
        print('colors_to_check options: Any combo of: "mb", "st", "lt", "bg", "gp')
        print('Example: py chupke_mokhtiyar.py "mb", "st", "lt"')
        sys.exit(1)

    trader = PIXEL_TRADER(colors_to_check)
    trader.run()